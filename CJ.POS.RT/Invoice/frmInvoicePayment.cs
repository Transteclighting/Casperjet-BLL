...
namespace CJ.POS.RT.Invoice
{
    public partial class frmInvoicePayment : Form
    {
            ...
            double CPDiscount, int nUicontrol, double totalRSamount, DSSalesInvoiceDiscount oSingleDiscount, double _AdditionalDis, double _TotalCharge, double _CollectionAmt, DSInvoicePayment _oDSInvoicePayment, bool _IsApplicableB2BDiscountInv, double TPDiscount, DSInvoicePayment _oDSExptProductID,int _FreeEMIID, bool bIsApplicableB2BDiscount,int nInvoiceQty)
        {
            ...
            oTELLib = new TELLib();
            _dtInstrumentDate = oTELLib.ServerDateTime();

            ConsumerPromotionEngine oExchangeOffer = new ConsumerPromotionEngine();
            _bPromoExchangeOffer = oExchangeOffer.GetExchangeOfferPromoRT(oTELLib.ServerDateTime().Date//DateTime.Now.Date
               , Utility.WarehouseID,
                _nProductID);
            ...
            if (oScretchCardOffer.GetScretchCardOfferCustomerTypeRT(oTELLib.ServerDateTime().Date//DateTime.Now.Date
                , Utility.WarehouseID, nProductiD, nUicontrol, nCustomeriD))
            {
                //OfferTypeSC
            ...
            if (ValidateUIInput())
            {
                ...
                foreach (DataGridViewRow oItemRow in dgvInvoicePayment.Rows)
                {
                    ...
                    if (oItemRow.Index < dgvInvoicePayment.Rows.Count)
                    {
                        ...
                        {
                            oInvoicePaymentRow.InstrumentDate = oTELLib.ServerDateTime().Date;//DateTime.Now.Date;
                        }
                        try
                        {
        ...
        private void lblAddPayment_Click(object sender, EventArgs e)
        {
            ...
            if (Convert.ToDouble(txtDueAmount.Text) > 0)
            {
                ...
                frmSinglePayment oForm = new frmSinglePayment(Convert.ToDouble(txtDueAmount.Text), lblProductName.Text, _nProductID, _nCustomerID, _nUIControl, _sConsumerCode,
                    -1, 0, "", oTELLib.ServerDateTime().Date, -1, -1, -1, -1, "", 0, -1, 0, 0, 1, oDSInvoicePaymentExptProductSingle, 0, nFreeEMIID, "");
                oForm.ShowDialog();
        ...
        private void dgvInvoicePayment_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex == 18)
            {
                ...
                {
                    _dtInstrumentDate = oTELLib.ServerDateTime().Date;
                }
                try
                {
        ...
        private void GetB2BDiscount()
        {
            ...
            _B2BDiscount = 0;
            _B2BDiscount = oB2BDiscount.GetB2BDiscountRT(_nCustomerID, _TotalRsAmount, oTELLib.ServerDateTime().Date);
        }
        private void GetB2CDealerDiscount()
        {
            ...
            double _Discount = 0;
            double _DMargin = 0;
            _Discount = oB2BDiscount.GetB2CDealerDiscountRT(_nUIControl, _nProductID, oTELLib.ServerDateTime().Date, _TotalRsAmount);

            _DMargin = oB2BDiscount.GetCustTypeMarginDiscountRSPRT(_nCustomerID, _nProductID, oTELLib.ServerDateTime().Date, _nInvoiceQty);

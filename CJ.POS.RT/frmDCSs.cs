...
namespace CJ.POS.RT
{
    public partial class frmDCSs : Form
    {
        ...
        private void DataLoadControl()
        {
            ...
            lvwDCSs.Items.Clear();
            {
                _oDCSs.RefreshRT(dtFromDate.Value.Date, dtToDate.Value.Date, txtDCSNo.Text, IsCheck);
            }
        ...
        private void btnPreview_Click(object sender, EventArgs e)
        {
            if (lvwDCSs.SelectedItems.Count != 0)
            {
                ...
                _oDCS.RefreshDCSDetailHO(1, _oDCS.DCSDate.Date,Utility.CustomerID);//_oDCS.RefreshDCSDetailRT(1, _oDCS.DCSDate.Date);

                _oDCSs = new DCSs();
                _oDCSs.RefreshHO1(_oDCS.DCSDate.Date,Utility.CustomerID);//_oDCSs.RefreshRT(Convert.ToDateTime(_oDCS.DCSDate.Date));

                DCS oDeposit = new DCS();
                oDeposit.GetDepositDetailHO(_oDCS.DCSID,Utility.CustomerID);//oDeposit.GetDepositDetailRT(_oDCS.DCSID);
                ...
                _oDCSDetail.GetAmountByTypeRT(_oDCS.DCSDate.Date);
                _oDCSDetail.GetCollectionAmountRT(_oDCS.DCSID);


                DCSDetail _oExtendedWarranty = new DCSDetail();
                _oExtendedWarranty.GetExtendedWarrantyAmountRT(_oDCS.DCSID);

                double _CreditCollection = _oDCSDetail.CreditCollection;

                PaymentMode oPaymentMode = new PaymentMode();
                oPaymentMode.GetInvoicePaymentModeRT(_oDCS.DCSDate.Date);

                double _AdvanceReceive = _oConsumerBalanceTran.GetAdvanceAmountByDateRT(_oDCS.DCSDate.Date);
                ...
                doc.SetParameterValue("PaymentDetail", _oDCSPaymentDetail.GetDcsPaymentDetailRT(_oDCS.DCSID));
                ...
                double _BGAmount = _BG.GetRevInvBGRT(_oDCS.DCSDate.Date);

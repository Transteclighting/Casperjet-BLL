...
namespace CJ.POS.RT
{
    public partial class frmTDDeliveryShipment : Form
    {
        ...
        public frmTDDeliveryShipment(int nType)
        {
            InitializeComponent();
            _nType = nType;
        }
        ...
        public void ShowDialog(TDDeliveryShipment oTDDeliveryShipment)
        {
            ...
            sInvoiceNo = oTDDeliveryShipment.InvoiceNo;
            ...
            if (_nType == (int)Dictionary.TDDeliveryShipmentStatus.Undelivered)
            {
                oTDDeliveryShipments.GetDataForUnDeliverdRT(sInvoiceNo);
            }
            else
            {
            ...
            if (_nType == (int)Dictionary.TDDeliveryShipmentStatus.Processing_Delivery || _nType == (int)Dictionary.TDDeliveryShipmentStatus.Undelivered)
            {
        ...
        private void frmTDDeliveryShipment_Load(object sender, EventArgs e)
        {
            ...
            foreach (DataGridViewRow oItemRow in dgvTDDeliveryShipment.Rows)
            {
                if (oItemRow.Index < dgvTDDeliveryShipment.Rows.Count)
                {
                    ...
                    if (oItemRow.Cells[6].Value != null)
                    {
                        try
                        {
                            double tmp = Convert.ToDouble(oItemRow.Cells[6].Value);
                        }
                        catch
                        {
                    ...
                    if (oItemRow.Cells[7].Value != null)
                    {
                        try
                        {
                            double tmp = Convert.ToInt32(oItemRow.Cells[7].Value);
                        }
                        catch
                        {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            if (validateUIInput())
            {
                try
                {
                    ...
                    oTDDeliveryShipment.EditRT(nShipmentID, _nType);
        ...
        public TDDeliveryShipment GetData(TDDeliveryShipment _oTDDeliveryShipment)
        {
            ...
            _oTDDeliveryShipment.CreateDate = oLib.ServerDateTime().Date;
            _oTDDeliveryShipment.CreateUserID = Utility.UserId;
            _oTDDeliveryShipment.UpdateDate = oLib.ServerDateTime().Date;

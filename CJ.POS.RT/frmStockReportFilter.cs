...
namespace CJ.POS.RT
{
    public partial class frmStockReportFilter : Form
    {
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            if (_nUIControl == 1)
            {
                ...
                else if (rdoCentralStock.Checked == true)
                {
                    ...
                    // if (CheckInternet())
                    {
                        WSReport(Utility.WarehouseID, Utility.CentralRetailWarehouse);
                    }
                    else
                    {
                        MessageBox.Show("Please Check Internet Connection\nSystem Date: " + Convert.ToDateTime(_oTELLib.ServerDateTime().Date).ToString("dd-MMM-yyyy") + "", "Error", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        this.Close();
                    }
                ...
                else
                {
                    ...
                    // if (CheckInternet())
                    {
                        WSReport(Utility.WarehouseID, 0);
                    }
                    else
                    {
                        MessageBox.Show("Please Check Internet Connection\nSystem Date: " + Convert.ToDateTime(_oTELLib.ServerDateTime().Date).ToString("dd-MMM-yyyy") + "", "Error", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private void WSReport(int nWHID, int nCentralWHID)
        {
            ...
            oProductStocks.GetWSStockRT(nWHID, nCentralWHID, nAGID, nASGID, nMAGID, nBrandID, txtProductCode.Text.Trim(), txtProductName.Text.Trim());
            if (nCentralWHID == 0)
            {
                ...
                doc.SetParameterValue("Outlet", "All Outlets Other than " + Utility.Shortcode + "");
                ...
                doc.SetParameterValue("PrintDate", _oTELLib.ServerDateTime().Date.ToString());//oSystemInfo.OperationDate.ToString());
            ...
            else
            {
                ...
                doc.SetParameterValue("ProductCode", txtProductCode.Text);
                doc.SetParameterValue("ProductName", txtProductName.Text);
                doc.SetParameterValue("PrintDate", _oTELLib.ServerDateTime().Date.ToString());
        ...
        private void Report(bool IsProdCheck)
        {
            ...
            oProductStocks.GetCurrentStockNewRT(cmbQuantity.SelectedIndex, IsProdCheck, Utility.WarehouseID, nMAGID, nASGID, nAGID, nBrandID, txtProductCode.Text.Trim(), txtProductName.Text.Trim());
        ...
        private void BarcodeReport()
        {
            ...
            //oSystemInfo.Refresh();
            oProductBarcodes.GetProductSerialRT(nType, nMAGID, nASGID, nAGID, nBrandID, txtProductCode.Text.Trim(), txtProductName.Text.Trim(), txtBarcode.Text.Trim());
        ...
        private void frmStockReportFilter_Load(object sender, EventArgs e)
        {
            ...
            if (_nUIControl == 1)
            {
                ...
                rdoOwnStock.Text = Utility.Shortcode + " Stock";

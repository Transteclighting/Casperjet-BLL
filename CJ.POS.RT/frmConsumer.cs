...
namespace CJ.POS.RT
{
    public partial class frmConsumer : Form
    {
        ...
        public frmConsumer(int nSalestype, int nCustomerID)
        {
            ...
            dtDateofBirth.Value = oLibrary.ServerDateTime();
        }
        ...
        public bool ValidateUIInput()
        {
            ...
            if (cmbSalesType.SelectedIndex + 1 == (int)Dictionary.SalesType.Dealer || cmbSalesType.SelectedIndex + 1 == (int)Dictionary.SalesType.B2B)
            {
                ...
                if (txtCtlCustName.Text != "")
                {
                    ...
                    {
                        RetailConsumer oConsumer = new RetailConsumer();
                        if (oConsumer.GetConsumerByCustomerIDRT(_oCustomer.CustomerID))
                        {
            ...
            if (ValidateUIInput())
            {
                ...
                else
                {
                    ...
                    try
                    {
                        ...
                        nMotherConsumerID = oRetailConsumer.GetConsumerDetailByMobileNo(oRetailConsumer.CellNo);
                        oRetailConsumer.ConsumerCode = oRetailConsumer.GetMaxConsumerCode();
                       
                        if (nMotherConsumerID == 0)
                        {
                            nMotherConsumerID = oRetailConsumer.AddConsumerDetailRT();
                        }
                        oRetailConsumer.MotherConsumerID = nMotherConsumerID;

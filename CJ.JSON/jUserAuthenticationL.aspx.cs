...
public partial class jUserAuthentication : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            BLLProductGroups _oBLLProductGroups = new BLLProductGroups();
            BLLBrands _oBLLBrands = new BLLBrands();
           /* BLLRegions _oBllRegions = new BLLRegions();*/
            ...
            oResponse.BLLProductGroupList = _oBLLProductGroups.GetBLLProductGroup();
            oResponse.BLLBrandList = _oBLLBrands.GetBLLBrand();
            //oResponse.BLLRegionList = _oBllRegions.GetBllRegions();
    ...
    public class Data
    {
        ...
        public List<BLLProductGroup> BLLProductGroupList;
        public List<BLLBrand> BLLBrandList;
        /*public List<BLLRegion> BLLRegionList;*/

        public void Authentication(string sUserName, string sPassword, string sIMEI, string sSIMSerial,
            string sAndroidAppID)
        {
            DBController.Instance.OpenNewConnection();
            User oUser = new User();
                                            ...
                                            if (oUser.CheckSIMSerialCJApps(sSIMSerial, sUserName))
                                            {
                                                if (oUser.AndroidAppID != (int) Dictionary.AndroidAppID.All)
                                                {
                                                    ...
                                                    else
                                                    {
                                                        ...
                                                        CheckRegistration = "false";
                                                        Message =
                                                            "You have no permission to use the App!!\nYou can only use " +
        ...
        public void GetOtherInfo(string sUserName, string sIMEI)
        {
            ...
            string sSQL = " select BLLMarketGroup from dbo.t_A_UserRegistration Where UserName='" + sUserName +
                          "' and IMEINo = '" + sIMEI + "' ";
            try
            {
    ...
    public class MarketGroups : CollectionBase
    {
        ...
        public List<MarketGroup> GetMarketGroup(string sAndroidAppID, string sUser)
        {
            ...
            string sSQL = " Select Distinct 'BLL' as Company, AreaID, AreaShortName as AreaName, AreaSort, " +
                          " TerritoryID, TerritoryShortName as TerritoryName, TerritorySort,RegionID,RegionSort,RegionName,RegionShortName" +
                          " from BLLSysDB.dbo.v_CustomerDetails Where ChannelID=2 ";

            if (nAndroidAppID == (int) Dictionary.AndroidAppID.CJ_Lighting)
            {
                sSQL = sSQL +
                       " and CustomerID IN ( select DataID from BLLSysDB.dbo.t_UserPermissionData Where DataType='Customer' and UserID = " +
            ...
            try
            {
                ...
                while (reader.Read())
                {
                    ...
                    oMarketGroup.RegionID = reader["RegionID"].ToString();
                    oMarketGroup.RegionSort = reader["RegionSort"].ToString();
                    oMarketGroup.RegionName = reader["RegionName"].ToString();
                    oMarketGroup.RegionShortName = reader["RegionShortName"].ToString();
    ...
    public class BLLDistributors : CollectionBase
    {
        ...
        public List<BLLDistributor> GetBLLDistributor(string sAndroidAppID, string sUser)
        {
            ...
            InnerList.Clear();
            string sSQL =
                " Select 'BLL' Company, CustomerID, CustomerCode, CustomerName, Isnull(CustomerShortName,CustomerName) as CustomerShortName, " +
            ...
            {
                sSQL = sSQL +
                       " and a.CustomerID IN ( select DataID from BLLSysDB.dbo.t_UserPermissionData Where DataType='Customer' and UserID = " +
                       " (Select UserID from BLLSysDB.dbo.t_User Where UserName='" + sUser + "')) ";
            }
            sSQL = sSQL +
                   " Group by CustomerID, CustomerCode, CustomerName, CustomerShortName, TerritoryID, a.IsActive  " +
    ...
    /*public class BLLRegions : CollectionBase
    {

        public BLLRegion this[int i]
        {
            get { return (BLLRegion) InnerList[i]; }
            set { InnerList[i] = value; }
        }


        public List<BLLRegion> GetBllRegions()
        {
            List<BLLRegion> eList = new List<BLLRegion>();
            OleDbCommand cmd = DBController.Instance.GetCommand();
            InnerList.Clear();
            string sSQL = "Select Distinct RegionShortName,RegionName from v_CustomerDetails";
            try
            {
                cmd.CommandText = sSQL;
                cmd.CommandType = CommandType.Text;
                ...
                while (reader.Read())
                {
                    BLLRegion oBLLRegion = new BLLRegion();
                    oBLLRegion.RegionName = reader["RegionName"].ToString();
                    oBLLRegion.RegionShortName = reader["RegionShortName"].ToString();
                    eList.Add(oBLLRegion);
                }
                reader.Close();
            }
            catch (Exception ex)
...
public class BLLProductGroups : CollectionBase
    {

        public BLLProductGroup this[int i]
        {
            get { return (BLLProductGroup)InnerList[i]; }
            set { InnerList[i] = value; }
        }

        public void Add(BLLProductGroup oBLLProductGroup)
        {
            InnerList.Add(oBLLProductGroup);
        }

        
    

        public List<BLLProductGroup> GetBLLProductGroup()
        {
            List<BLLProductGroup> eList = new List<BLLProductGroup>();
            OleDbCommand cmd = DBController.Instance.GetCommand();
            InnerList.Clear();
            string sSQL = " Select 'BLL' as Company, PdtGroupID as ProductGroupID, PdtGroupName as ProductGroupName, PdtGroupType as ProductGroupType, " +
                          " IsNull(ParentID,0) as ParentID, POS as Sort from BLLSysDB.dbo.t_ProductGroup where IsActive=1 Order by PdtGroupType, POS ";
            try
            {
                cmd.CommandText = sSQL;
                cmd.CommandType = CommandType.Text;
                ...
                while (reader.Read())
                {
                    BLLProductGroup oTELProductGroup = new BLLProductGroup();
    ...
    public class BLLBrands : CollectionBase
    {

        public BLLBrand this[int i]
        {
            get { return (BLLBrand)InnerList[i]; }
            set { InnerList[i] = value; }
        }

        public void Add(BLLBrand oTELBrand)
        {
            InnerList.Add(oTELBrand);
        }
        public List<BLLBrand> GetBLLBrand()
        {
            List<BLLBrand> eList = new List<BLLBrand>();
            OleDbCommand cmd = DBController.Instance.GetCommand();
            InnerList.Clear();
            string sSQL = " Select 'BLL' as Company,BrandID, BrandDesc as BrandName from BLLSysDB.dbo.t_Brand Where IsActive=1 and BrandLevel=1 Order by BrandPos ";
            try
            {
                cmd.CommandText = sSQL;
                cmd.CommandType = CommandType.Text;
                ...
                while (reader.Read())
                {
                    BLLBrand oTELBrand = new BLLBrand();

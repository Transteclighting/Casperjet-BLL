...
namespace CJ.Win
{
    ...
    /// </summary>
    public partial class ctlJob : System.Windows.Forms.UserControl
    {
        ...
        /// </summary>
        public ReplaceJobFromCassandra SelectedReplaceJobFromCassandra
        {
            get
            {
                //if (_oEmployee == null)
        ...
        private void ctlJob_Resize(object sender, EventArgs e)
        {
            if (this.Width <= 180)
            {
                this.Left = this.Left;
        ...
        private void btnPicker_Click(object sender, EventArgs e)
        {
            ...
            frmJobSearch frmJobSearch = new frmJobSearch();
            frmJobSearch.ShowDialog(_oReplaceJobFromCassandra);
           
            if (_oReplaceJobFromCassandra.JobNo != null)
            {
                txtCode.Text = _oReplaceJobFromCassandra.JobNo;
            }
        }
        private void txtCode_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                _oReplaceJobFromCassandra.JobNo = txtCode.Text;
                _oReplaceJobFromCassandra.RefreshByJobNo();

                if (_oReplaceJobFromCassandra.CustomerName == null)
                {
                    _oReplaceJobFromCassandra = null;
                    AppLogger.LogFatal("There is no data.");
                    return;
                }
                else
                {
                    txtDescription.Text = _oReplaceJobFromCassandra.CustomerName.ToString();
